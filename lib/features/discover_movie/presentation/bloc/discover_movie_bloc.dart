...
class DiscoverMovieBloc extends Bloc<DiscoverMovieEvent, DiscoverMovieState> {
  ...
  DiscoverMovieBloc(this.discoverMovieUseCase) : super(DiscoverMovieInitialState()) {
    on<DiscoverMovieEvent>((event, emit) {
    ...
    on<GetMovieDiscoverEvent>((event , emit)async{
      ...
      result.fold((l) {
        ...
        emit(state.copyWith(status: ScreenStatus.success, movieDiscoverModel: r));
      } );
